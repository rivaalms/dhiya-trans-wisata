...
<admin-layout :title="product?.name || 'Produk'">
   ...
   <div v-if="product" class="grid grid-cols-4 gap-4 place-items-start">
      <template v-for="image in product.images">
         <div class="relative">
            ...
            <div class="absolute inset-x-0 top-0 flex justify-end gap-1 p-2">
               <button class="bg-amber-500/70 px-3 py-1 text-xs text-white font-semibold rounded hover:bg-amber-500 hover:shadow hover:shadow-amber-300 transition-all duration-300" @click.stop="showModal(image.uuid)">
